 ...
 */
class Import extends \WP_Importer {
	...
	function import( $file ) {
		...
		$this->process_tags();
		$this->process_terms();
		$this->process_pricing_categories();
		$this->process_posts();
	...
	function import_start( $file ) {
		...
		$this->pricing_categories = $import_data['pricing_categories'];
		$this->trips              = $import_data['items'];
	...
	function import_options() {
		$j = 0;
		?>
		<form action="<?php echo admin_url( 'admin.php?import=wptravelengine-import-export&amp;step=2' ); ?>" method="post">
	...
	function process_pricing_categories() {
		if ( empty( $this->pricing_categories ) ) {
			return;
		}

		foreach ( $this->pricing_categories as $term ) {
			...
			if ( $term_id ) {
				...
				if ( isset( $term['term_id'] ) ) {
					$this->processed_pricing_categories[ intval( $term['term_id'] ) ] = (int) $term_id;
				}
				if ( isset( $term->is_primary ) && $term->is_primary ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset( $term['term_id'] ) ) {
					$this->processed_pricing_categories[ intval( $term['term_id'] ) ] = $id['term_id'];
					if ( isset( $term->is_primary ) && $term->is_primary ) {
						update_option( 'primary_pricing_category', $id['term_id'] );
					}
				}
			} else {
				printf( __( 'Failed to import %1$s %2$s', 'wordpress-importer' ), esc_html( $term['taxonomy'] ), esc_html( $term['name'] ) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
					echo ': ' . $id->get_error_message();
				}
				echo '<br />';
	...
	protected function process_termmeta( $term, $term_id ) {
		...
		foreach ( $term['meta'] as $meta_key => $meta_value ) {
			 ...
			 * @param array  $term     Term data from the WXR import.
			 */
			// $key = apply_filters( 'import_term_meta_key', $meta_key, $term_id, $term );
			if ( ! $meta_key ) {
				continue;
			}
			...
			add_term_meta( $term_id, wp_slash( $meta_key ), wp_slash_strings_only( $value ) );
		}
	}

	private function process_packages( $packages ) {
		$packages_ids = array();
			...
			if ( $new_package_id ) {
				...
				if ( is_array( $package_catgories ) ) {
					foreach ( $package_catgories as $key => $value ) {
						if ( is_array( $value ) ) {
							foreach ( $value as $c_id => $_value ) {
								if ( ! isset( $this->processed_pricing_categories[ $c_id ] ) ) {
									continue;
								}
				...
				update_post_meta( $new_package_id, 'package-categories', $_package_categories );
				$packages_ids[] = $new_package_id;
			}
		}
		return $packages_ids;
	}
	...
	private function process_single_post( $post ) {
		...
		if ( 'nav_menu_item' == $post['post_type'] ) {
			...
			$post_exists = 'trip-packages' !== $post['post_type'] && post_exists( $post['title'], '', $post['post_date'] );
		...
		if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
			printf( __( '%1$s &#8220;%2$s&#8221; already exists.', 'wordpress-importer' ), $post_type_object->labels->singular_name, esc_html( $post['title'] ) );
			echo '<br />';
			$comment_post_ID                                = $post_id = $post_exists;
			$this->processed_posts[ intval( $post['ID'] ) ] = intval( $post_exists );
		} else {
			if ( isset( $post['packages'] ) ) {
				if ( is_array( $post['packages'] ) ) {
					$packages_ids = $this->process_packages( $post['packages'] );
				}
			}
		...
		if ( ! empty( $post['comments'] ) ) {
			...
			foreach ( $newcomments as $key => $comment ) {
				...
				if ( ! $post_exists || ! comment_exists( $comment['comment_author'], $comment['comment_date'] ) ) {
					...
					foreach ( $comment['commentmeta'] as $meta_key => $meta_value ) {
						$value = maybe_unserialize( $meta_value );

						add_comment_meta( $inserted_comments[ $key ], wp_slash( $meta_key ), wp_slash_strings_only( $value ) );
					}
		...
		// $post['meta'] = apply_filters( 'wp_import_post_meta', $post['meta'], $post_id, $post );

			// add/update post meta
		if ( ! empty( $post['meta'] ) ) {
			foreach ( $post['meta'] as $key  => $meta_value ) {
				// $key   = $meta['key'];
				$value = false;

				if ( '_edit_last' == $key ) {
					if ( isset( $this->processed_authors[ intval( $meta_value ) ] ) ) {
						$value = $this->processed_authors[ intval( $meta_value ) ];
					} else {
						$key = false;
					}
				}
	...
	function process_posts() {
		$this->trips = apply_filters( 'wptravelengine_import_trips', $this->trips );

		foreach ( $this->trips as $post ) {
	...
	function parse( $file ) {
		...
		if ( ! $data ) {
			return new \WP_Error( 'INVALID_FILE', 'Invalid file or file is corrupted' );
		}
		return $data;
	}
	...
	function greet() {
		...
		echo '<p>' . __( 'Howdy! Upload your exporting file which is exported using WP Travel Engine - Import Export Plugin.', 'wordpress-importer' ) . '</p>';
		echo '<p>' . __( 'Choose a JSON (.json) file to upload, then click Upload file and import.', 'wordpress-importer' ) . '</p>';
		wp_import_upload_form( 'admin.php?import=wptravelengine-import-export&amp;step=1' );
		echo '</div>';
	}
...
add_action(
	'admin_init',
	function() {
		$importer_instance = new Import();
		register_importer(
			'wptravelengine-import-export',
			'WP Travel Engine - Importer',
			__( 'Import <strong>Trips</strong> from a WordPress.', 'wordpress-importer' ),
			array( $importer_instance, 'dispatch' )
		);
	}
);
